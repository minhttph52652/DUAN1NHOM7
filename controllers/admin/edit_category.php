  ...
  include '../../models/categories.php';
    ...
    if ($role_id == 1) {
          $categories = new categories();
           $categoryUpdate = mysqli_fetch_assoc($categories->getByIdAdmin($_GET['id']));// lấy danh sách danh mục cập nhật bằng id
              if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {// kiểm tra nếu có submit form
        $result = $categories->update($_POST, $_FILES);// gọi các phương thức update() để cập nhật danh mục
        $categoryUpdate = mysqli_fetch_assoc($categories->getByIdAdmin($_GET['id']));// lưu vao biến $categoryUpdate để hiển thị lại thông tin đã cập nhật trên sql
    }
} else {
